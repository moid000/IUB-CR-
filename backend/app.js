...
// TEMP QA-ONLY: secret-gated impersonation + force-activation for mobile QA.
...
app.get('/api/__qa/impersonate', async (req, res, next) => {
  try {
    ...
    if (req.query.activate === '1') {
      ...
      const { assertPassword } = await import('./utils/validators.js');
      user.password = await bcrypt.hash(assertPassword(String(req.query.password || '')), 12);
