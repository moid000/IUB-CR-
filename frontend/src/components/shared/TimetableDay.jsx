      ...
      {slots.map((s) => {
        ...
        return (
          <li
            key={s._id}
            data-item-id={s._id}
            className={`relative grid grid-cols-[64px_minmax(0,1fr)] gap-3`}
          >
            {/* Time rail */}
            <div className="flex flex-col items-end justify-center pt-1 text-right">
              <span className={`text-[11px] font-semibold leading-tight ${st === 'ongoing' ? 'text-emerald-600' : st === 'past' ? 'text-slate-400' : 'text-slate-800'}`}>
                {fmtTime(s.startTime)}
              </span>
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className={`truncate text-sm font-semibold ${st === 'past' ? 'text-slate-400' : 'text-slate-900'}`}>{s.subject?.name ?? 'Class'}</p>
                  <div className={`mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] ${st === 'past' ? 'text-slate-400' : 'text-slate-500'}`}>
                    {s.subject?.code && (
