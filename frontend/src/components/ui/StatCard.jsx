...
export function StatCard({ to, icon: Icon = null, label, value, hint, className = '', wideMobile = false }) {
  const top = (
    <div className="flex min-w-0 items-center gap-2.5">
      ...
      <p className="min-w-0 line-clamp-2 break-words text-[10px] font-semibold uppercase leading-tight tracking-[0.1em] text-slate-500">{label}</p>
    </div>
  );
  ...
  const metric = (
    <div className={wideMobile ? 'min-w-0 text-right lg:text-left' : 'min-w-0'}>
      ...
      {hint && <p className="mt-1 line-clamp-2 break-words text-[10px] leading-tight text-slate-400">{hint}</p>}
    </div>
  );
