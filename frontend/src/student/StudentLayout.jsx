...
import {
  ...
  IconQr, IconCheckCircle, IconBell, IconMenu, IconX, IconLogout, IconGraduation,
  IconUserSquare,
...
const NAV = [
  ...
  { to: '/student/marks', label: 'Marks', icon: IconCheckCircle },
  { to: '/student/notifications', label: 'Notifications', icon: IconBell, badge: 'unread' },
];

function pageTitle(pathname) {
  const match = NAV.find((n) => n.to !== '/student' && pathname.startsWith(n.to));
  return match ? match.label : 'Dashboard';
}

/**
 * Student portal shell — mobile-first, mirroring the CR layout. Section
 * context is ALWAYS server-derived from the authenticated student.
 */
    ...
    <div className="min-h-dvh bg-slate-50">
      ...
      >
        <div className="flex h-16 shrink-0 items-center border-b border-slate-100 px-5">
          <NavLink to="/student" className="flex items-center gap-2.5 rounded-lg py-1">
              ...
              {menuOpen && (
                ...
                >
                  <div className="border-b border-slate-100 px-4 py-3">
                    ...
                    <Badge variant="primary" className="mt-1.5">Student</Badge>
                  </div>
                  <NavLink to="/student/profile" role="menuitem" className="block px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50">
                    Profile
                  </NavLink>
                  <button
