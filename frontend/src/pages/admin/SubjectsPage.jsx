    ...
    <FormModal open={open} onClose={onClose} title={isEdit ? 'Edit subject' : 'New subject'} submitLabel={isEdit ? 'Save changes' : 'Create subject'} onSubmit={submit} size="lg">
      {(fieldErrors) => (
        <>
          ...
          <Textarea id="subject-description" rows={3} label="Description (optional)" value={description} onChange={(e) => setDescription(e.target.value)} error={fieldErrors?.description} placeholder="Brief outline of the subject…" />
        </>
      )}
    </FormModal>
  );
